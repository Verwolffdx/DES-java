...
public class DES {
    ...
    public static byte[] encrypt_CBC (byte[] data, byte[] key, byte[] init_vector) {
        int lenght=0;
        byte[] padding = new byte[1];
        int i;
        lenght = 8 - data.length % 8;
        ...
        for (i = 0; i < data.length + lenght; i++) {
            if (i > 0 && i % 8 == 0) {
//                bloc = xor_func(bloc, init_vector);
                bloc = encrypt64Bloc(xor_func(bloc, init_vector),K, false);
                System.arraycopy(bloc, 0, init_vector, 0, bloc.length);
                System.arraycopy(bloc, 0, tmp, i - 8, bloc.length);
            }
            if (i < data.length)
                bloc[i % 8] = data[i];
...
//        }
    ...
    public static byte[] decrypt_CBC(byte[] data, byte[] key, byte[] init_vector) {
        int i;
        byte[] tmp = new byte[data.length];
        byte[] bloc = new byte[8];
        ...
        for (i = 0; i < data.length; i++) {
            if (i > 0 && i % 8 == 0) {
                bloc = xor_func(init_vector, encrypt64Bloc(bloc,K, true));

                System.arraycopy(bloc, 0, tmp, i - 8, bloc.length);
                System.out.println("DEBUG: " + new String(tmp));
                System.arraycopy(bloc, 0, init_vector, 0, bloc.length);
            }
            if (i < data.length)
                bloc[i % 8] = data[i];
